...
func main() {
	...
	fmt.Printf("1. %s ($%.2f limit) - Hash: %s\n", resp.Data.Label, resp.Data.Limit, resp.Data.Hash)
	fmt.Printf("2. %s ($%.2f limit, BYOK included) - Hash: %s\n", resp2.Data.Label, resp2.Data.Limit, resp2.Data.Hash)
	fmt.Printf("3. %s (no limit) - Hash: %s\n", resp3.Data.Label, resp3.Data.Hash)
	fmt.Println()
	fmt.Println("⚠️  Remember to save the API key values shown above!")
	fmt.Println("⚠️  They will not be displayed again.")
	fmt.Println()

	// Example: Deleting an API key (commented out for safety)
	fmt.Println("=== Deleting API Keys ===")
	fmt.Println("To delete a key programmatically:")
	fmt.Println()
	fmt.Println("  // ⚠️ WARNING: This is irreversible!")
	fmt.Println("  // deleteResult, err := client.DeleteKey(ctx, resp.Data.Hash)")
	fmt.Println("  // if err != nil {")
	fmt.Println("  //     log.Fatal(err)")
	fmt.Println("  // }")
	fmt.Println("  // if deleteResult.Data.Success {")
	fmt.Println("  //     fmt.Println(\"API key deleted successfully\")")
	fmt.Println("  // }")
	fmt.Println()
	fmt.Printf("You can also delete keys manually at: https://openrouter.ai/settings/keys\n")
}
